...
const CANONICAL_DIRECTORY = 'docs/animation-events/v3-9-days-mvp';
...
const WIKI_URL = 'https://gitlab.com/scout-gg/croupier/-/wikis/animation-events/v3-9-days-mvp/';
...
test('imports all specified mirrors, maps GitHub names and wiki slugs, and preserves unrelated files', t => {
  ...
  for (const relative of ['animation-events/v3-9-days-mvp/s03-eventtradet.md', 'animation-events-v3/canvas/s03-eventtradet.md', 'animation-events-v3-s03.md']) {
...
test('HTML copies are byte-identical and manifest is deterministic without private paths or timestamps', t => {
  ...
  assert.ok(html.equals(fs.readFileSync(path.join(f.wiki, 'animation-events-v3.html'))));
  assert.ok(html.equals(fs.readFileSync(path.join(f.wiki, 'animation-events-v3/canvas/index.html'))));
  const manifestPath = path.join(f.github, 'docs/sync-manifest.json');
  const initial = fs.readFileSync(manifestPath, 'utf8');
  assert.equal(JSON.parse(initial).generatorVersion, '1.0.0');
...
test('rejects a symlink escaping the wiki root before touching mirrors', t => {
  ...
  fs.symlinkSync(outside, path.join(f.wiki, 'animation-events-v3'), process.platform === 'win32' ? 'junction' : 'dir');
...
test('rejects a file in a target directory position before any output is written', t => {
  const f = fixture(t);
  fs.writeFileSync(path.join(f.wiki, 'animation-events-v3'), 'Not a directory\n');
  const before = snapshot(f.temporary);
  const result = f.sync('--write');
  assert.equal(result.status, 1);
