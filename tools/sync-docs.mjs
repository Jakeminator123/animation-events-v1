#!/usr/bin/env node
/**
 * One-way, offline V3 mirror builder. Edit Croupier's canonical Markdown, not
...
function buildPlan(sourceRoot, githubRoot) {
  ...
  for (const [name, destination] of MAPPING) {
    ...
    for (const prefix of [CANONICAL_DIRECTORY.replace(/^docs\//, ''), 'animation-events-v3/canvas']) {
      const wikiDestination = `${prefix}/${name}`;
      wiki.set(wikiDestination, Buffer.from(mirrorMarkdown(content, name, wikiDestination, 'wiki')));
    }
    if (/^s\d\d-/.test(name)) {
      const wikiDestination = `animation-events-v3-${name.slice(0, 3)}.md`;
      wiki.set(wikiDestination, Buffer.from(mirrorMarkdown(content, name, wikiDestination, 'wiki')));
    }
  }
  const html = readRequired(githubRoot, 'index.html');
  github.set('index.html', html);
  wiki.set('animation-events-v3.html', html);
  wiki.set('animation-events-v3/canvas/index.html', html);
  const manifest = {
    schemaVersion: 1,
...
function expectedWikiPaths() {
  ...
  for (const [name] of MAPPING) {
    paths.push(`${CANONICAL_DIRECTORY.replace(/^docs\//, '')}/${name}`, `animation-events-v3/canvas/${name}`);
    if (/^s\d\d-/.test(name)) paths.push(`animation-events-v3-${name.slice(0, 3)}.md`);
  }
  return [...paths, 'animation-events-v3.html', 'animation-events-v3/canvas/index.html'];
}
